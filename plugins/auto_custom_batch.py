...
async def custom_batch(client: Client, message: Message):
    ...
    hi = await message.reply_text("Send the message or media you want to store:")
    first = await client.listen(user_id)
    if first.text:
        session_data[user_id]["messages"].append({"type": "text", "content": first.text.strip()})
    elif first.media:
        session_data[user_id]["messages"].append({
            "type": "media",
            "media_type": first.media,
            "file_id": first[first.media].file_id
        })
    else:
        await hi.edit("Unsupported message type. Please send text or media.")
        return
    await hi.delete()
    await first.delete()
    sent_message = await message.reply_text(
        ...
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("Add Message/Media", callback_data=f"add_message_{user_id}")],
...
async def add_message_handler(client: Client, callback_query: CallbackQuery):
    ...
    prompt = await callback_query.message.reply_text("Send the next message or media you want to store:")
    new_message = await client.listen(user_id)

    # Store text or media (using file_id)
    if new_message.text:
